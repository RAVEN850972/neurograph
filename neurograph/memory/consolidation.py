...
class ConsolidationOrchestrator:
    ...
    def _perform_consolidation(self) -> None:
        ...
        with self._lock:
            try:
                # Проверяем, есть ли что консолидировать
                if len(self.stm) == 0:
                    return
    ...
    def force_consolidation(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Принудительно перемещаем элементы из STM в LTM при переполнении
            items_to_consolidate = min(len(self.stm), max(1, len(self.stm) - self.stm_capacity + 1))
            
            if items_to_consolidate > 0:
                # Берем самые старые элементы для консолидации
                oldest_items = sorted(self.stm.items(), key=lambda x: x[1].created_at)[:items_to_consolidate]
                
                for item_id, item in oldest_items:
                    # Перемещаем из STM в LTM
                    self.stm.pop(item_id)
                    ...
                    if self.semantic_index and item.embedding is not None:
                        try:
                            ...
                            logger.warning(f"Не удалось добавить элемент в семантический индекс: {str(e)}")
            ...
            consolidated_count = before_stats["stm_size"] - after_stats["stm_size"]
            
            return {
                "before": before_stats,
                "after": after_stats,
                "consolidated": max(0, consolidated_count),  # Не может быть отрицательным
                "timestamp": time.time()
            }
