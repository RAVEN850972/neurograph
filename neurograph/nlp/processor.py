...
class StandardNLProcessor(INLProcessor):
    ...
    def __init__(self, 
                 ...
                 **kwargs):  # Добавляем **kwargs для дополнительных параметров
...
class LightweightNLProcessor(INLProcessor):
    ...
    def __init__(self, language: str = "ru", **kwargs):  # Добавляем **kwargs
        self.language = language
...
class AdvancedNLProcessor(StandardNLProcessor):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)  # Передаем все kwargs родительскому классу
